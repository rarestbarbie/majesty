...
extension CandlestickChart {
    mutating func update(with market: Market, date: GameDate) {
        guard let first: Market.Interval = market.history.first else {
            self.history = []
            self.min = 0
            self.max = 1
            return
        ...
        for interval: Market.Interval in market.history.reversed() {
