...
struct PopSnapshot: PopProperties, Sendable {
    ...
    let portfolio: [(asset: LEI, value: Double)]
...
extension PopSnapshot {
    init(
        ...
        portfolio: [(asset: LEI, value: Double)],
    ) {
...
extension PopSnapshot {
    func tooltipAccount(_ account: Bank.Account) -> Tooltip {
        return .instructions {
            ...
            let portfolioValue: Delta<Double> = self.Δ.portfolioValue
            let inventoryValue: Delta<Int64> = self.Δ.assets
            let illiquidValue: Delta<Double> = portfolioValue + inventoryValue.map(
                Double.init(_:)
            )
            let totalNetWorth: Delta<Double> = illiquidValue + account.Δ.map(Double.init(_:))

            $0["Total net worth", +] = totalNetWorth[/3..2]
            $0[>] {
                if  case .Elite = self.occupation.stratum {
                    $0["Portfolio value", +] = portfolioValue[/3..2]
                    $0["Physical assets", +] = inventoryValue[/3]
                } else {
                    $0["Illiquid assets", +] = illiquidValue[/3..2]
                }
            }
...
extension PopSnapshot {
    func tooltipPortfolioValue(account: Bank.Account) -> Tooltip {
        .instructions {
            $0["Per-capita portfolio value", +] = self.Δ.μ.portfolioValue[/3..2]
            $0[>] {
                let weight: Double = Double.init(self.z.total)
                let account: Mean<Bank.Account> = .init(fields: account, weight: weight)
                $0["Dividends received", +] = +?account.i[/3..2]
            }

            $0["Total portfolio value", +] = self.Δ.portfolioValue[/3..2]
            if  case .Elite = self.occupation.stratum {
                $0[>] {
                    $0["Stock purchases", +] = +?(-account.e)[/3]
                    $0["Stock sales", +] = +?(-account.f)[/3]
                }
            }

            $0[>] = """
            The portfolio value is updated daily by marking the value of each asset \
            to the current market price regardless of liquidity
            """
        }
    }

    func tooltipPortfolio(in cache: borrowing GameUI.Cache, id: CountryID) -> Tooltip? {
        guard
        let country: Country = cache.countries[id] else {
            return nil
        }

        return .instructions {
            let fraction: Ratio<Double> = self.aggregatePortfolioValue {
                id == cache.countryResidence(of: $0)
            }
            $0[country.name.short] = (fraction.defined ?? 0)[%3]
            $0[>] {
                $0["Estimated \(self.region.currency.name) value", +] = fraction.selected[/3..3]
            }
            $0[>] = """
            The wealth generated by overseas investments counts towards the GDP of the country
            the asset is located in, and the GNP of the country where the investor resides
            """
        }
    }

    func tooltipPortfolio(in cache: borrowing GameUI.Cache, id: EconomicLedger.Industry) -> Tooltip {
        return .instructions {
            let fraction: Ratio<Double> = self.aggregatePortfolioValue {
                id == cache.industryClassification(of: $0)
            }
            $0[cache.rules.name(id)] = (fraction.defined ?? 0)[%3]
            $0[>] {
                $0["Estimated \(self.region.currency.name) value", +] = fraction.selected[/3..3]
            }
            $0[>] = """
            Investors are more likely to choose investments with a higher \
            \(em: "expected return")
            """
        }
    }

    private func aggregatePortfolioValue(
        where predicate: (LEI) -> Bool,
    ) -> Ratio<Double> {
        self.portfolio.reduce(into: .zero) {
            if  predicate($1.asset) {
                $0.selected += $1.value
            }
            $0.total += $1.value
        }
